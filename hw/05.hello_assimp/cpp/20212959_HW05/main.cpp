...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
  ...
  if (key == GLFW_KEY_S && action == GLFW_PRESS && mods == GLFW_MOD_CONTROL){
    ...
    for (std::size_t i = 0; i < g_objects.size(); ++i){
      ...
      std::string name = g_objects[i].get_name();
      glm::vec3 scale = g_objects[i].get_scale();
      glm::vec3 translate = g_objects[i].get_translate();
    ...
    for (std::size_t i = 0; i < g_cameras.size(); ++i){
      ...
      glm::vec3 position = g_cameras[i].position();
      glm::vec3 at_position = g_cameras[i].position() + g_cameras[i].front_direction();
      glm::vec3 up_direction = g_cameras[i].up_direction();
